...
d = world.device  # single source of truth for device
...
class LightGCN(nn.Module):
    ...
    def _init_weight(self):
        ...
        self.embedding_user1 = torch.nn.Embedding(self.num_users, self.latent_dim)
        self.embedding_user2 = torch.nn.Embedding(self.num_users, self.latent_dim)
        self.embedding_item = torch.nn.Embedding(self.num_items, self.latent_dim)
        self.embedding_social = torch.nn.Embedding(self.num_users, self.latent_dim)
        ...
        # Interaction graphs (sparse) -> ensure coalesced & on device
        self.interactionGraph, self.interactionGraph2 = (
            self.dataset.getInteractionGraph()
        )
        self.interactionGraph = self.interactionGraph.coalesce().to(d)
        self.interactionGraph2 = self.interactionGraph2.coalesce().to(d)
...
class CLDS(LightGCN):
    def _init_weight(self):
        ...
        # Social graphs -> device
        self.socialGraph1 = self.dataset.getSocialGraph1().coalesce().to(d)
        self.socialGraph2 = self.dataset.getSocialGraph2().coalesce().to(d)
        ...
        # Initial shuffle indices on device
        a, b = list(range(world.n_node)), list(range(world.n_node))
        np.random.shuffle(a)
        np.random.shuffle(b)
        self.shuffle_index1 = torch.tensor(a, device=d, dtype=torch.long)
        self.shuffle_index2 = torch.tensor(b, device=d, dtype=torch.long)
    ...
    def computer(self, epoch):
        ...
        if epoch < 2000:
            ...
            for layer in range(self.n_layers):
                ...
                users_emb_social = torch.sparse.mm(S1, users_emb)
        ...
        np.random.shuffle(a)
        np.random.shuffle(b)
        shuffle_index1 = torch.tensor(a, device=d, dtype=torch.long)
        shuffle_index2 = torch.tensor(b, device=d, dtype=torch.long)
    ...
    def bpr_loss(self, users, pos, neg, epoch):
        ...
        # attribute decorrelation (as in your original)
        M_ = users_emb * neg_emb
        M1 = torch.pow(torch.mean(M_), 2)
        M2 = torch.mean(torch.pow(M_, 2)) - torch.tensor(
            1.0 / float(self.latent_dim), device=users_emb.device
        )
        ...
        if tag == 1:
            ...
            lbl_1 = torch.ones((1, logits.detach().shape[1] // 2), device=d)
            lbl_2 = torch.zeros(lbl_1.shape, device=d)
            lbl = torch.cat((lbl_1, lbl_2), 1)  # no .cuda(); keep device-agnostic
